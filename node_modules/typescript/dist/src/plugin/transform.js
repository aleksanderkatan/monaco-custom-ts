 ...
 */
export function transformSourceFile(options) {
        ...
        return (sourceFile) => {
            function visit(node) {
                ...
                if (ts.isBinaryExpression(node)) {
                    const runtimeFn = opToRuntimeFn[node.operatorToken.kind];
                    ...
                    if (shouldTransformBinaryExpression(node, checker) && runtimeFn) {
                        ...
                        const callExpression = ts.factory.createCallExpression(ts.factory.createIdentifier(`__tsover_${runtimeFn}`), undefined, // type arguments
                        [left, right]);
